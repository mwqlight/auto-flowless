...
export const constantRoutes: RouteRecordRaw[] = [
	...
	// AI大模型管理
	{
		...
		redirect: '/llm/upload',
		name: 'LLM',
		meta: {title: '大模型管理', icon: 'chatbot'}, // 可以根据实际情况选择合适的图标
		children: [
			{
				path: 'upload',
				component: () => import('@/views/llm/upload.vue'),
				name: 'LLMUpload',
				meta: {title: '模型上传', icon: 'upload'}
			},
			{
				path: 'download',
				component: () => import('@/views/llm/download.vue'),
				name: 'LLMDownload',
				meta: {title: '模型下载', icon: 'download'}
			},
			{
				path: 'train',
				component: () => import('@/views/llm/train.vue'),
				name: 'LLMTrain',
				meta: {title: '模型训练', icon: 'train'}
			},
			{
				path: 'evaluate',
				component: () => import('@/views/llm/evaluate.vue'),
				name: 'LLMEvaluate',
				meta: {title: '模型评估', icon: 'evaluate'}
			}
		]
	}
];
